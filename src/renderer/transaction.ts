...
const pancakeContractAddress = '0x9ac64cc6e4415144c455bd8e4837fea55603e5c3';
export const getBalance = async (address: string) => {
  return web3.eth.getBalance(address);
};
export const getCode = (token: string) => {
  return web3.eth.getCode(web3.utils.toChecksumAddress(token));
};
export const getTransactionCount = (address: string) => {
  return web3.eth.getTransactionCount(address, 'pending');
};

export const pancakeContractABI = [
  {
    inputs: [
  ...
  },
  {
    ...
    name: 'swapExeactTokensForTokens',
    outputs: [
      { internalType: 'uint256[]', name: 'amounts', type: 'uint256[]' },
    ],
    stateMutability: 'nonpayable',
...
export const pancakeContract = new web3.eth.Contract(
  pancakeContractABI,
  pancakeContractAddress
);
export const startSniping = async (
  _address: string,
  balance: string,
  amount: string
) => {
